...
def preprocessing(line):
    ...
    if user_trace[-1] == '\n':
        user_trace.pop()  # final element is null
    ...
    if len(user_stay) <= stay_threshold:
...
def identify_home(user_stay):
    ...
    if len(res) > 0 and float(sum([x[1] for x in res])) > 0:
        r = res[0][1] / float(sum([x[1] for x in res]))
    else:
        r = 0
    if r > home_threshold:
...
def merge_same(user_trace):
    ...
    for point in user_trace:
        if last_place and distance(last_place, point[0:2]) == 0:
...
def filter_active(info):
    if info['user_stay'] > active_user_stay and info['home_stay'] > active_home_stay:
        return True
    else:
        return False
...
def individual_nw(info):
    ...
    nw = nw * 7.0 / (info['end_day'] - info['start_day'])
    return nw


# Calculate parameters of global: P_t
def global_rhythm(info):
    rhythm = [0] * 7 * 24 * (60 / time_slot)
...
# reduce function for global parameter
...
# simulate traces
def simulate_traces(info, rhythm_global):
    ...
    beta1 = beta_id / 20
    beta2 = (beta_id - beta1 * 20) * 5
    info['beta'] = [beta1 + 1, beta2 + 1]
    info['n_w'] = n_w
    return info
...
# predict the next place based on the temporal and spatial selection
def predict_next_place(info, rhythm_global, delta_r):
    n_w = info['n_w']
    beta1, beta2 = info['beta']
    deltar = delta_r.value[0]
    rhythm = rhythm_global.value[0]
    location_type = ['home', 'work', 'other']
    current_location = info['stays'][-1][1][0:2]
    current_location_type = location_type[info['stays'][-1][0]]
    ti = stamp2array(info['stays'][-1][1][3])
    time_id = int(ti.tm_wday * 24 * int(60.0 / time_slot) + ti.tm_hour * int(60.0 / time_slot) + ti.tm_min / time_slot)

    # TODO:location type is in conflict
    next_location_type, location_change = predict_next_place_time(n_w, rhythm[time_id], beta1, beta2,
                                                                  current_location_type)
    if location_change:
        next_location = predict_next_place_location(info, deltar, current_location)
    else:
        next_location = [current_location_type, current_location]
    info['next_location'] = next_location
    return info
